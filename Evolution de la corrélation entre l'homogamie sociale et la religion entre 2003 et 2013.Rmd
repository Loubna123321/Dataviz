...
Les sciences sociales, se sont depuis longtemps intéressées à l'homogamie dans la mise en couple. 
...
Voyons en premier lieu la répartition des personnes en couple:
```{r}
ggplot(hdv) +
  aes(x = Vie_en_couple) +
...
hdv %>%
  ...
  aes(x = Religiosite) +
  geom_bar(fill = "#0C4C8A") +
  labs(x = "Croyance et pratiques",title = "La question de pratiques et croyance réligieux pour les individus en couple en 2003") +
  theme_bw() +
...
On peut constater qu'à partir de 55ans, les personnes se déclarent moins croyantes, toutefois, les personnes non croyantes reste inférieur aux personnes croyantes. 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! C'est faux : On voit que les personnes qui se déclarent le plus 'non-croyantes' sont les jeunes ! (colonne de droite), s'il y a de moins en moins de points après 55 ans ( je dirais meme plutot 65) dans la colonne de gauche... c'est parcequ'il y a de moins en moins de personnes vu... qu'elles meurent...
...
# Répartition Age en fonction de la croyance  en 2013 :
```{r}
epic %>%
